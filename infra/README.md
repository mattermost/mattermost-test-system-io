...
| **Admin key** | AWS Secrets Manager (`<project>-{staging,production}-admin-key`) | Auto-generated by CDK; mounted as `TSIO_ADMIN_KEY`. Gates `POST /api/v1/auth/oidc-policies` |
...
one endpoint today: `POST /api/v1/auth/oidc-policies`, which inserts rows into
...
curl -fsS -X POST https://test-io.test.mattermost.com/api/v1/auth/oidc-policies \
