...
└── AppStack (shared VPC, ALB, ECS cluster — ~$168/mo)
    ...
    │   ├── ECS PostgreSQL 18.3 (ephemeral, Cloud Map discovery, Secrets Manager password)
...
| **Session signing secret** | AWS Secrets Manager (`<project>-{staging,production}-session-secret`) | Auto-generated by CDK (`generateSecretString`); mounted as `TSIO_SESSION_SECRET` |
| **Admin key** | AWS Secrets Manager (`<project>-{staging,production}-admin-key`) | Auto-generated by CDK; mounted as `TSIO_ADMIN_KEY`. Gates `POST /api/v1/admin/oidc-policies` |
...
| Session signing secret | Manual — `aws secretsmanager put-secret-value` + `aws ecs update-service --force-new-deployment`. Invalidates all live cookies on cutover. |
| Admin key | Manual — same `put-secret-value` + `force-new-deployment` flow as the session secret (see below) |

### Admin key & OIDC policies

The admin key (`TSIO_ADMIN_KEY`) is a bootstrap-only credential. It gates exactly
one endpoint today: `POST /api/v1/admin/oidc-policies`, which inserts rows into
`github_oidc_policies`. Once a policy row exists, normal traffic authenticates
via short-lived GitHub Actions OIDC bearer tokens (or `X-API-Key` for
non-OIDC callers) — the admin key is never on the hot path.

**Where it comes from.** CDK creates the secret on first deploy:
...
`generateSecretString` only runs at create — `cdk deploy` never overwrites an
existing value.
...
aws secretsmanager get-secret-value \
  --secret-id mattermost-test-system-io-staging-admin-key \
  --query SecretString --output text
```

**Rotating** (no grace window — the cutover is hard-cut):
...
aws ecs update-service \
  ...
  --service mattermost-test-system-io-staging-staging-test-io \
  --force-new-deployment
```

ECS only re-reads task secrets at task start, so `--force-new-deployment` is
required. Anyone holding the old value loses access at the moment their task
gets replaced.

**Seeding policies on staging.** Staging recreates its Postgres task on every
deploy, which would otherwise wipe `github_oidc_policies`. The staging stack
sets `TSIO_BOOTSTRAP_OIDC_POLICIES="mattermost/*=uploader"`; the server reads
this on startup and runs `INSERT ... ON CONFLICT (name) DO NOTHING` for each
entry. Format is comma-separated `pattern=role` (e.g.
`mattermost/*=uploader,otherorg/specific-repo=editor`). Look for
`bootstrap oidc policy ensured` in the staging container logs after each
deploy.

**Seeding policies on production.** Production uses RDS, so policies persist
across deploys — bootstrap-on-startup isn't needed. Seed once via curl:

```bash
ADMIN_KEY=$(aws secretsmanager get-secret-value \
  --secret-id mattermost-test-system-io-production-admin-key \
  --query SecretString --output text)

curl -fsS -X POST https://test-io.test.mattermost.com/api/v1/admin/oidc-policies \
  -H "X-Admin-Key: $ADMIN_KEY" \
  -H "Content-Type: application/json" \
  -d '{"repository_pattern":"mattermost/*","role":"uploader"}'
```

A duplicate POST returns `409 ALREADY_EXISTS` — safe to retry.

**Audience.** `TSIO_GITHUB_ACTIONS_OIDC_AUDIENCE` is set to
`mattermost-test-system-io` on both staging and production; CI workflows must
mint their OIDC tokens with the same `audience=` parameter. Mismatch → token
verification fails with 401, no orchestration access.
