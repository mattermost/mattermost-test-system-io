...
// CreateOIDCPolicy serves POST /api/v1/admin/oidc-policies.
//
// Gated by X-Admin-Key — the admin auth scheme for /admin/* endpoints. The
// standard X-API-Key / Bearer / session contract applies to user and CI
// endpoints; admin operations like installing OIDC policies authenticate via
// the long-lived admin key (auto-generated by CDK in staging/production and
// stored in Secrets Manager). May be called many times to install policies
// for additional orgs or repos.
//
// Installs a github_oidc_policies row from a repository_pattern shorthand.
//
