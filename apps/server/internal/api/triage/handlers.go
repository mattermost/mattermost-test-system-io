// Package triage serves /api/v1/triage/* — the read APIs an automated E2E
// triage agent consults, and the ledger it writes back to.
//
// Test System IO deliberately forms no opinion about a failure. It stores what
// happened and answers questions about it; the agent that classifies, fixes and
// verifies runs elsewhere, where it can build and run a Mattermost server and
// prove a fix by running the test. The one exception is `attribution.go`, which
// is arithmetic over stored history rather than judgement.
//
// Three things depend on the ledger:
//
//   - Attribution: was this failure already happening on master? That answer
//     lets a pull request stop paying for someone else's flake.
...
//   - Memory: what has already been tried on a test, so the agent does not
//     re-attempt what it has already failed.
package triage
...
type verdictRow struct {
	...
	// Evidence is the citations that justified the verdict. It is returned
	// rather than stored-and-forgotten because this list IS the audit surface:
	// a waiver whose evidence cannot be read back is indistinguishable from a
	// waiver that had none.
	Evidence         []json.RawMessage `json:"evidence"`
...
func (h *Handlers) ListVerdicts(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.Pool.Query(r.Context(), `
		SELECT id, repository, branch, commit_sha, gh_run_id, gh_pr_number,
		       ...
		       root_cause, evidence, suspect_commit, check_state, waived, model,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&v.ID, &v.Repository, &v.Branch, &v.CommitSHA, &v.GHRunID,
			...
			&v.Verdict, &v.Confidence, &v.RootCause, &v.Evidence, &v.SuspectCommit,
...
// normalizeRepo accepts either a bare name ("mattermost") or a full slug
// ("mattermost/mattermost"). Reports are ingested under the full slug, so bare
// names are widened rather than rejected: every caller here is a CI job or an
// agent that knows the repository by one name or the other, not both.
