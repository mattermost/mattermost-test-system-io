 ...
 *
 * `/reports/begin` is idempotent (the controller called it earlier); a
 * second call returns the same report_group_id.
 */
...
export async function uploadShard(
  ...
  const reportsIdent: ReportsBeginBody = identityForReports(cfg.compositeIdentity);
...
function identityForReports(c: CompositeIdentity): ReportsBeginBody {
