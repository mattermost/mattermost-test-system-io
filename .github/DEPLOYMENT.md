...
- The run ID makes every promotion's Docker tag / GitHub release unique, even when re-promoting without bumping `VERSION` first (e.g. re-running a failed promotion, or promoting a follow-up beta of the same version). This removes the requirement to bump `VERSION` before every production deploy — you still bump it for real semver-significant changes (see below), but a same-version re-promotion no longer collides with or overwrites the previous release's tag.
