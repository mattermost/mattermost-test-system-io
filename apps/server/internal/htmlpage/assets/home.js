(function () {
  ...
  var repoSelect = document.getElementById('filter-repo');
  var branchSelect = document.getElementById('filter-branch');

  function navigateWithFilters(updates) {
    var params = new URLSearchParams(location.search);
    if (updates.repository !== undefined) {
      if (updates.repository) params.set('repository', updates.repository);
      else params.delete('repository');
      params.delete('branch_filter');
    }
    if (updates.branchFilter !== undefined) {
      if (updates.branchFilter) params.set('branch_filter', updates.branchFilter);
  ...
  function selectedRepository() {
    if (repoSelect && repoSelect.value) return repoSelect.value;
    return new URLSearchParams(location.search).get('repository') || '';
  }
  ...
  if (repoSelect) {
    repoSelect.addEventListener('change', function () {
      navigateWithFilters({ repository: repoSelect.value });
    });
  }
  if (branchSelect) {
    branchSelect.addEventListener('change', function () {
      navigateWithFilters({ branchFilter: branchSelect.value });
    });
  }
